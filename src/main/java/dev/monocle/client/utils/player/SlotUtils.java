...
import dev.monocle.client.mixin.AbstractMountInventoryMenuAccessor;
import dev.monocle.client.mixin.CreativeModeInventoryScreenAccessor;
import dev.monocle.client.mixin.CreativeModeTabsAccessor;
...
public class SlotUtils {
    ...
    private static int creativeInventory(int i) {
        if (CreativeModeInventoryScreenAccessor.monocle$getSelectedTab() != BuiltInRegistries.CREATIVE_MODE_TAB.getValue(CreativeModeTabsAccessor.monocle$getInventory()))
            return -1;
    ...
    private static int horse(AbstractContainerMenu handler, int i) {
        LivingEntity entity = ((AbstractMountInventoryMenuAccessor) handler).monocle$getMount();
