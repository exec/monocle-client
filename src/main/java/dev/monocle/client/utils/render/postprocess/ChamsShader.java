...
public class ChamsShader extends EntityShader {
    ...
    public ChamsShader() {
        super(MonocleRenderPipelines.POST_IMAGE);
        MonocleClient.EVENT_BUS.subscribe(ChamsShader.class);
    }
    ...
    public static void load() {
        try {
            ...
            for (String fileFormat : FILE_FORMATS) {
                Optional<Resource> optional = mc.getResourceManager().getResource(MonocleClient.identifier("textures/chams." + fileFormat));
            ...
            MonocleClient.LOG.error("Error loading the chams shader", e);
        }
    }
    ...
    private static final DynamicUniformStorage<UniformData> UNIFORM_STORAGE = new DynamicUniformStorage<>("Monocle - Image UBO", UNIFORM_SIZE, 16);
