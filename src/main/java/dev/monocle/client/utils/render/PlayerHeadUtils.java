...
import dev.monocle.client.systems.accounts.TexturesJson;
import dev.monocle.client.systems.accounts.UuidToProfileResponse;
...
public class PlayerHeadUtils {
    ...
    public static byte[] fetchHead(UUID id) {
        ...
        try {
            ...
            MonocleClient.LOG.error("Could not fetch player head for {}.", id, e);
            return null;
        }
    }
    ...
    public static String getSkinUrl(UUID id) {
        UuidToProfileResponse res2 = Http.get("https://sessionserver.mojang.com/session/minecraft/profile/" + id)
            .exceptionHandler(e -> MonocleClient.LOG.error("Could not contact mojang session servers.", e))
