...
import dev.monocle.client.mixin.CrossbowItemAccessor;
import dev.monocle.client.mixin.ProjectileInGroundAccessor;
...
public class ProjectileEntitySimulator {
    ...
    public boolean set(Entity user, ItemStack itemStack, double angleOffset, boolean accurate, float tickDelta) {
        ...
        switch (item) {
            ...
            case CrossbowItem _ -> {
                ...
                float speed = CrossbowItemAccessor.monocle$getSpeed(projectilesComponent);
    ...
    public boolean set(Entity entity) {
        ...
        if (entity instanceof ProjectileInGroundAccessor ppe && ppe.monocle$invokeIsInGround()) return false;
    ...
    public SimulationStep tick() {
        tickCount++;
        ((IVec3) prevPos3d).monocle$set(pos);
    ...
    private SimulationStep getCollision() {
        ...
        if (blockCollision.getType() != HitResult.Type.MISS) {
            ((IVec3) pos3d).monocle$set(blockCollision.getLocation());
        }
