...
import dev.monocle.client.mixin.ItemStackRenderStateAccessor;
import dev.monocle.client.mixin.LayerRenderStateAccessor;
...
public class ItemPhysics extends Module {
    ...
    private void onRenderItemEntity(RenderItemEntityEvent event) {
        ...
        for (int i = 0; i < ((ItemStackRenderStateAccessor) event.renderState.item).monocle$getActiveLayerCount(); i++) {
            ItemStackRenderState.LayerRenderState layer = ((ItemStackRenderStateAccessor) event.renderState.item).monocle$getLayers()[i];
            ModelInfo info = getInfo(layer.prepareQuadList());

            matrices.pushPose();
            applyTransformation(matrices, ((LayerRenderStateAccessor) layer).monocle$getTransform());
