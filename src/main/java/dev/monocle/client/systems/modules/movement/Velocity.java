...
import dev.monocle.client.mixin.ClientboundSetEntityMotionPacketAccessor;
...
public class Velocity extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if ((mc.player.isInWater() || mc.player.isInLava()) && mc.player.getDeltaMovement().y < 0) {
            ((IVec3) mc.player.getDeltaMovement()).monocle$setY(0);
        }
    }
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        if (knockback.get() && event.packet instanceof ClientboundSetEntityMotionPacket packet
            ...
            ((ClientboundSetEntityMotionPacketAccessor) (Object) packet).monocle$setMovement(
