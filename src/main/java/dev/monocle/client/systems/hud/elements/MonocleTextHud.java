...
public class MonocleTextHud {
    public static final HudElementInfo<TextHud> INFO = new HudElementInfo<>(Hud.GROUP, "text", "Displays arbitrary text with Starscript.", MonocleTextHud::create);
    ...
    static {
        ...
        MODULE_ENABLED = addPreset("Module enabled", "Kill Aura: {monocle.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"}", 0);
        MODULE_ENABLED_WITH_INFO = addPreset("Module enabled with info", "Kill Aura: {monocle.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"} #1{monocle.get_module_info(\"kill-aura\")}", 0);
        WATERMARK = addPreset("Watermark", "{monocle.name} #1{monocle.version}");
