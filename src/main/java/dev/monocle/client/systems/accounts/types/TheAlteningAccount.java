...
public class TheAlteningAccount extends Account<TheAlteningAccount> implements TokenAccount {
    ...
    public boolean fetchInfo() {
        try {
            ...
            if (res == null || res.accessToken == null || res.selectedProfile == null) {
                MonocleClient.LOG.error("Invalid TheAltening credentials.");
                return false;
            }
            ...
            MonocleClient.LOG.error("Failed to fetch info for TheAltening account!");
            return false;
        }
    }
    ...
    public boolean login() {
        ...
        try {
            ...
            MonocleClient.LOG.error("Failed to login with TheAltening.");
            return false;
        }
    }
    ...
    private AuthResponse authenticate() {
        return Http.post(ENVIRONMENT.servicesHost() + "/authenticate")
            .bodyJson(new AuthRequest("MINECRAFT", token, "Monocle Client", UUID.randomUUID().toString(), true))
