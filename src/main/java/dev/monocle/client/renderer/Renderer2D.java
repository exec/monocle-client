...
import dev.monocle.client.gui.renderer.packer.TextureRegion;
...
public class Renderer2D {
    ...
    public Renderer2D(boolean textured) {
        ...
        triangles = new MeshBuilder(textured ? MonocleRenderPipelines.UI_TEXTURED : MonocleRenderPipelines.UI_COLORED);
        lines = new MeshBuilder(MonocleRenderPipelines.UI_COLORED_LINES);
    }
    ...
    public void render(String samplerName, GpuTextureView samplerView, GpuSampler sampler) {
        ...
        MeshRenderer.begin()
            ...
            .pipeline(MonocleRenderPipelines.UI_COLORED_LINES)
            .mesh(lines)
            .end();

        MeshRenderer.begin()
            .attachments(Minecraft.getInstance().gameRenderer.mainRenderTarget())
            .pipeline(textured ? MonocleRenderPipelines.UI_TEXTURED : MonocleRenderPipelines.UI_COLORED)
            .mesh(triangles)
    ...
    public void triangle(double x1, double y1, double x2, double y2, double x3, double y3, Color c1, Color c2, Color c3) {
        ...
        triangles.triangle(
            triangles.vec2(x1, y1).color(c1).next(),
            triangles.vec2(x2, y2).color(c2).next(),
            triangles.vec2(x3, y3).color(c3).next()
        );
    }
